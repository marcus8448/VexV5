...
void move_for(double right_distance, double left_distance, int32_t velocity) {
    ...
    double d = fabs(motor_rf.get_position() - motor_rf.get_target_position());
    while (fabs(motor_rf.get_position() - motor_rf.get_target_position()) > 8 || fabs(motor_lb.get_position() - motor_lb.get_target_position()) > 8) {
        double e = velocity * std::min((fabs((motor_rf.get_position() - motor_rf.get_target_position())) / d) + 0.7, 1.0);
        if (e < velocity && e > 0) {
            velocity = (int32_t) e;
            motor_rf.modify_profiled_velocity(velocity);
            motor_rb.modify_profiled_velocity(velocity);
            motor_lf.modify_profiled_velocity(velocity);
            motor_lb.modify_profiled_velocity(velocity);
        }
        delay(50);
    }
    delay(100);
...
void arm_down(int32_t velocity) {
    ...
    delay(400);
    while (fabs(arm.get_position() - arm.get_target_position()) > 8) {
        delay(50);
    }
}
