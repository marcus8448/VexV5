...
DataSet::DataSet(const char *label,
                 lv_color_t color,
...
template<size_t Sets, size_t Points> Chart<Sets, Points>::Chart(robot::Robot &robot,
                                                            const char *title,
...
template<size_t Sets, size_t Points> void Chart<Sets, Points>::initialize(lv_obj_t *screen) {
  this->canvasBuffer = malloc(LV_CANVAS_BUF_SIZE_TRUE_COLOR(SCREEN_WIDTH, SCREEN_HEIGHT));
...
template<size_t Sets, size_t Points> void Chart<Sets, Points>::update() {
  memset(canvasBuffer, 0x00000000, LV_CANVAS_BUF_SIZE_TRUE_COLOR(SCREEN_WIDTH, SCREEN_HEIGHT));
  ...
  lv_canvas_draw_text(this->drivetrainCanvas, 40, SCREEN_HEIGHT - 36, 100, &textDesc, str.c_str());
  ...
  for (size_t i = 0; i < Sets; ++i) {
    ...
    for (size_t j = 0; j < Points; j++) {
      ...
      points[j].y = static_cast<lv_coord_t>(SCREEN_HEIGHT - (this->data[i].get(Points - 1 - j) - min) * heightScale - 41);
    }
