...
namespace robot {
VelocityPid::VelocityPid(uint8_t port, double Kp, double Ki, double Kd, double dt): port(port), Kp(Kp), Ki(Ki), Kd(Kd), dt(dt), A0(Kp + Ki*dt), A1(-Kp), A0d(Kd/dt), A1d(- 2.0*Kd/dt), A2d(Kd/dt), tau(Kd / (Kp*5)), alpha(dt / (2*tau)) {
}
...
void VelocityPid::update() {
  if (this->targetVelocity == 0.0) return;
  ...
  if (this->output > 12000) this->output = 12000;
  // if (this->count++ % 100 == 0) {
