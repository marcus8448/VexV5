...
template<int PROVIDERS, Number Num> requires Max4<PROVIDERS> class Graph : public Screen {
private:
  const char *title;
  const char *xLabel;
  const char *yLabel;
...
Graph<PROVIDERS, Num>::Graph(robot::Robot &robot, const char *title, const char *xLabel, const char *yLabel, const Num max, const Num min,
                             DataProvider<Num> dataProviders[PROVIDERS]): title(title), xLabel(xLabel), yLabel(yLabel), max(max), min(min), heightScale(CANVAS_HEIGHT / (max - min)), widthScale(CANVAS_WIDTH / static_cast<float>(100)), robot(robot), dataProviders(dataProviders) {
